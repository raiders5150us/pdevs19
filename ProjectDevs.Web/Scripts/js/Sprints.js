...
$(function () {
    ...
    $(document).on("change", '#filter-team-id', function () {
        var storyTeamId = $('#filter-team-id').select2('data').map(function (a) { return a.id; }).toString();
    ...
    var createEditSprintModal_Adtn = $('#create-edit-sprint-additional-modal');
    var saveSprintModalForm_Adtn = createEditSprintModal_Adtn.find('#sprints-form');
    var saveSprintBtn_Adtn = createEditSprintModal_Adtn.find('#save-sprint-btn');
    ...
    function openEditSprintPopupWithAdditionalDetails(sprintId) {
        if (sprintId === void 0) { sprintId = 0; }
        var sprintIdElement = createEditSprintModal_Adtn.find('[name=SprintId]');
        sprintIdElement.val(sprintId);
        if (sprintId > 0) {
            MainLoader.show();
            var sprintDetailUrl = UrlHelper.GetSprintsUrl(ApiUrl.SprintDetail, sprintId);
    ...
    function resetStorySprintAdditionalForm() {
        saveSprintModalForm_Adtn[0].reset();
    ...
    saveSprintBtn_Adtn.on('click', function (e) {
        var sprintId = saveSprintModalForm_Adtn.find('[name=SprintId]').val() || 0;
        MainLoader.show();
        if (sprintId > 0) {
            var sprintSaveUrl = UrlHelper.GetSprintsUrl(ApiUrl.SprintUpdate, sprintId);
